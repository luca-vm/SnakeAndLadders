...
type
  TfrmSnakesAndLadders = class(TForm)
    ...
    procedure bmbClosePanelClick(Sender: TObject);
  private
 iNum, iDice1, iDice2, iDice3, iTop, iTop2, iLeft, iLeft2, iCount2xPlayer1, iCount2xPlayer2, iNumPlayers: Integer;

  public
  iWin, iCount, iTurn : Integer;                     //Placed here to be used in other forms
...
procedure TfrmSnakesAndLadders.bmbClosePanelClick(Sender: TObject);
begin
  pnlRules.Visible := False;               //Closes rules page
end;
...
begin
  ...
  if iWin > 0           //Makes sure that no message is diplayed when the game is won
  then iWin := iWin
...
begin
   //Down button
   //Exits prevent tokens moving off the board
...
procedure TfrmSnakesAndLadders.btnRulesClick(Sender: TObject);
begin
  ...
  if iNumPlayers = 1
   ...
   if iNumPlayers = 1
    then
              ...
              if (sShape2 = 'Square') OR (sShape2 = 'square')
                then
                  shpToken2.Shape := stSquare
                    else if (sShape2 = 'Round square') OR (sShape2 = 'round square')
                       then
     ...
     if iNumPlayers = 1           //Prompts the player to move the dice for the 1st time
      then
      begin
...
begin
  btnDoneClick(Self);  {Allows the mystery box to give another instruction
                        if current one is impossible, Reference: Virtual Keys document}
end;

procedure TfrmSnakesAndLadders.tmrImpossibleMysTimer(Sender: TObject);
begin
  if lblImpossibleMys.Font.Color = clWhite            //Aloows for the flashing lable
